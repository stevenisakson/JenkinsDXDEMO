    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
        ...
        stage('Create Scratch Org') {
          ...
          stage('Deploy to Sandbox'){
              if (isUnix()) {
                rc = sh returnStatus: true, script: "${toolbelt} force:auth:jwt:grant --clientid 3MVG9Nc1qcZ7BbZ1y5WZhKhMOdnPsjqsrJQhgBIAAMCZLAa04qB2rCNVFNwyurzgyUZsiJIMowv6UlF_bEwIK --username steven.isakson@slalom.com.cancdci --jwtkeyfile ${jwt_key_file} --instanceurl https://test.salesforce.com"
                }else{
                 rc = bat returnStatus: true, script: "\"${toolbelt}\" force:auth:jwt:grant --clientid 3MVG9Nc1qcZ7BbZ1y5WZhKhMOdnPsjqsrJQhgBIAAMCZLAa04qB2rCNVFNwyurzgyUZsiJIMowv6UlF_bEwIK --username steven.isakson@slalom.com.cancdci --jwtkeyfile ${jwt_key_file} --instanceurl https://test.salesforce.com"
                }
              if (isUnix()) {
